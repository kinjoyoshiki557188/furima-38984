  ...
  describe '商品の出品登録' do
    ...
    context '出品ができないとき' do
      ...
      it '発送までの日数の情報が「---」だと出品できない' do
        @item.sipping_date_id = 0
        @item.valid?
        expect(@item.errors.full_messages).to include('Sipping date must be other than 0')
      end
      it '発送までの日数の情報が空欄だと出品できない' do
        @item.sipping_date_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Sipping date can't be blank", 'Sipping date is not a number')
      end
      it '価格が空欄だと出品できない' do
        @item.price = nil
        @item.valid?
